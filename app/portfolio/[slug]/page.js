...
import { ArrowRight, Calendar, Users, Wrench, Target, TrendingUp, Quote, Star } from "lucide-react";
...
export default function ProjectDetail({ params }) {
  ...
  const media = getPortfolioImage(slug);

  return (
    <Shell>
      <JsonLd data={[
        breadcrumbSchema([{ label: "Home", href: "/" }, { label: "Portfolio", href: "/portfolio" }, { label: p.title }]),
        caseStudySchema(p, media.hero),
      ]} />
      ...
      <section className="relative py-8"><div className="container"><Reveal>
        <div className="relative rounded-3xl overflow-hidden aspect-[16/7]">
          <img src={media.hero} alt={`${p.title} – case study cover`} className="absolute inset-0 w-full h-full object-cover" />
          <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-black/10 to-transparent" />
          <div className={`absolute inset-0 opacity-20 mix-blend-overlay bg-gradient-to-br ${p.grad}`} />
        </div>
      </Reveal></div></section>
      ...
      <section className="relative py-16"><div className="container">
        ...
        <div className="mt-14 grid grid-cols-1 md:grid-cols-3 gap-4">
          {[media.hero, ...media.gallery].slice(0, 6).map((src, n) => (
            <Reveal key={n} delay={n * 0.04}>
              <div className="aspect-video rounded-2xl overflow-hidden relative group">
                <img src={src} alt={`${p.title} – visual ${n + 1}`} loading="lazy" className="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" />
                <div className={`absolute inset-0 opacity-20 mix-blend-overlay bg-gradient-to-br ${p.grad}`} />
              </div>
            </Reveal>
          ))}
            ...
            return (
              <Reveal key={it.slug} delay={i * 0.05}>
                <Link href={`/portfolio/${it.slug}`} className="group relative block rounded-3xl overflow-hidden aspect-[4/3]">
                  <img src={img} alt={it.title} loading="lazy" className="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" />
