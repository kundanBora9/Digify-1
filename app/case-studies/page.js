  ...
  return (
    <Shell>
      ...
      <section className="relative py-16"><div className="container space-y-8">
        {portfolio.map((p, i) => {
          const img = getPortfolioImage(p.slug).hero;
          return (
            <Reveal key={p.slug} delay={Math.min(i, 4) * 0.06}>
              <Link href={`/portfolio/${p.slug}`} className="group grid md:grid-cols-2 gap-6 items-center glass rounded-3xl p-6 md:p-8 hover:bg-white/[0.06] transition">
                <div className="relative aspect-video rounded-2xl overflow-hidden">
                  <img src={img} alt={p.title} loading="lazy" className="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/40 to-transparent" />
                  <div className={`absolute inset-0 opacity-25 mix-blend-overlay bg-gradient-to-br ${p.grad}`} />
                </div>
                <div>
