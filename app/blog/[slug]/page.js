...
export default function BlogPost({ params }) {
  ...
  const prev = blogs[idx - 1]; const next = blogs[idx + 1];
  ...
  return (
    <Shell>
      ...
      <section className="relative py-4"><div className="container">
        <div className="relative aspect-[16/7] rounded-3xl overflow-hidden bg-gradient-to-br from-fuchsia-500 via-purple-600 to-indigo-600">
          <div className="absolute inset-0 mesh-bg opacity-40 mix-blend-overlay" />
          <div className="absolute inset-0 grid-lines opacity-30" />
          <div className="absolute inset-0 flex items-center justify-center"><b.icon className="w-32 h-32 text-white/90" /></div>
        </div>
      </div></section>
        ...
        <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-6">
          {related.map((r, i) => (
            <Link key={r.slug} href={`/blog/${r.slug}`} className="group block glass rounded-3xl overflow-hidden hover:bg-white/[0.06] transition">
              <div className="relative aspect-[16/10] bg-gradient-to-br from-fuchsia-500 via-purple-600 to-indigo-600"><div className="absolute inset-0 mesh-bg opacity-40 mix-blend-overlay" /><div className="absolute inset-0 flex items-center justify-center"><r.icon className="w-14 h-14 text-white/90" /></div></div>
